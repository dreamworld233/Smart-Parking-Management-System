...
describe('availabilityLevel', () => {
  ...
  it('档位与基调在整条数轴上一致，不依赖两边都引用同一个常量', () => {
    // 上面那条两侧都引用 FREE_FLOOR 符号，只钉得住「两边同源」。有人把某一侧写死成字面量时，
    // 它能否抓到取决于 SATURATION_THRESHOLD 往哪边动：阈值**下调**（旧字面量变得比真实下限大）
    // 时它照样绿 —— 这里补的就是这个方向；阈值上调时它自己会挂。两条合起来才覆盖两个方向。
    // 样本里字面量与 FREE_FLOOR 混用是有意的：0.12 这类字面量负责抓「一侧被写死」，
    // FREE_FLOOR 负责钉住当前边界本身。
