...
export const FREE_CANCEL_MS = 10 * 60 * 1000

/** 入场缓冲（毫秒）。到达时刻之后仍可核销；缓冲不算进预约时长，也不参与退费计算 */
export const ENTRY_GRACE_MS = 15 * 60 * 1000
 ...
 */
export function isBookableArrival(now: Date, arrive: Date): boolean {
  const diffMs = arrive.getTime() - now.getTime()
...
export interface CancelRefund {
  /** 已占用的计费小时数（向上取整）。免费取消窗口内为 0 */
  usedHours: number
  /** 退还的预支停车费（元）。下限 0，差额不追缴 */
  parkingRefund: number
  /** 是否退还平台服务费 */
  refundServiceFee: boolean
  /** 退款合计（元） */
  totalRefund: number
  /** 是否构成违约：到达时刻之后才取消，即「没有及时取消/到场」 */
  isBreach: boolean
}
