...
- **计费**
  - `预支停车费` = **预约时长（向上取整，不足 1 小时按 1 小时）× 该车场首小时标准价**，本质是**锁位费**
  - 例（万象城首小时 ¥6）：10:00 约 11:00 到 → `1 × 6 = ¥6`；约 12:00 到 → `2 × 6 = ¥12`；约 10:40 到 → `¥6`
  - 平台代收后转付车场；入场后实际停放费由车场闸机自行计收，**平台不参与、不抵扣、不追缴**
  - `平台服务费` = **¥2**，平台收入；取消时仅下单 10 分钟内全额退还才含此 ¥2
  - **退款**：`max(0, (预约时长 − 已占用时长)) × 首小时单价`，已占用 = 下单→取消（向上取整）
  - **不做**：自动扣费 / 免密代扣、钱包余额、改签
