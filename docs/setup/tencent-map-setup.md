...
**配额**：腾讯位置服务的 WebServiceAPI 个人开发者有每日免费额度（搜索、逆地址解析等各接口分别计数）。**地点搜索这一档很小（约 200 次/天）**，不是「课程演示规模远够用」——一次首页加载就是 1 次搜索，开发期反复刷新几天就能打光。`services/lot.ts` 因此把搜索结果缓存 10 分钟。具体数值以控制台「配额」页为准，官方会调整。
...
> ⚠️ **别拿真机调试来证明 B 步骤配好了**（2026-09-13 踩过）：真机调试跑的是**调试模式，微信在调试模式下放宽域名校验**。所以「工具里取消勾选后仍报 `不在以下 request 合法域名列表中`，真机却能拉到 POI」这个组合是**完全可能的**，而结论是**后台没配成**。判据只有两个：mp 后台的 request 列表本身，以及取消勾选 + 重编译后工具还报不报错（工具会缓存域名配置，可能要**重登/重启**才刷新）。**体验版与正式版不给任何豁免** —— 这条没配好，交付版本会是「地图有、车场一条没有」，比整页空白更难查。
...
| `status: 120` `此key每秒请求量已达到上限` | **路径矩阵按目的地计费，实测约 5 点/秒**：单发 5 个目的地可以、6 个就 120；同一秒内「3 点 + 3 点」第二发也 120。并发 20 个目的地会有大半被限流并静默退回估算值，所以 `walkingDistances` 拆批 + 留间隔，live 测试也必须串行 |
...
| 开发者工具里`<map>`整块空白，真机却正常 | **工具限制，不是代码问题**。控制台原文：`[Component] <map>: 开发者工具暂未支持 Skyline 下的 canvas 组件调试，请先到真机上预览调试。` Skyline 的 `map` 是 canvas 实现的，**开发者工具至今没有实现它的调试支持**（2026-09-13 实测，工具 2.02.2608070 / lib 3.17.2）。**别再花时间查 Key、域名、`.wrap` 高度** —— 地图相关的验收一律在真机上做，Task 0 的结论也是真机测的 |
| 真机调试控制台刷 `backgroundfetch privacy fail {"errno":101,...private_getBackgroundFetchData:fail:jsapi invalid request data}` | 微信客户端内部的**周期性更新**调用，代码里没有任何引用（已 grep 确认）。本项目没用这个能力，属噪音，可忽略 |
| `request:fail url not in domain list` | request 合法域名没配；开发期可勾「不校验合法域名」绕过 |
| `https://apis.map.qq.com 不在以下 request 合法域名列表中`（开发者工具控制台，`WAServiceMainContext.js` 抛的） | 同上，B 步骤未配。**注意它的杀伤面比看着大**：`services/qqmap.ts` 的搜索与距离矩阵全走 `wx.request`，域名没配时首页直接进 `state: 'error'`，卡片一条都出不来——地图瓦片是原生组件渲染的，不受这条影响，所以**只看到这条报错时别去查地图为什么不显示** |
