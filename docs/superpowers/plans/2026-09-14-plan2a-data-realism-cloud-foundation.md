...
| Task 6 页面接入 + 真机验收 | ✅（2026-09-15）—— 接入代码 Task 3 已顺带改完，本轮补两条空态文案、合成暂定值标注；真机 checklist 9 项全过。**期间用户拍板两项 2b 方向并已实现真机验证**：①目的地检索改 suggestion 候选（`89c9dcf` 起，交互三轮回调定稿于 `7786f1f`）②未签约车场显示仅导航（`b77f82a`）—— 详见 [[smart-parking-miniprogram-project]] 的 2b 段 | `eab75a1`、`89c9dcf`、`b77f82a`、`ef2f81f`、`a8c303d`、`7786f1f` |
...
| 未签约 POI 车场进列表 | **退出列表**（2026-09-14 拍板，**2026-09-15 反转**）：2a 期间哈希退役后未签约车场没有收费/余位来源；用户当日决定**未签约也展示、仅提供基础导航**（签约可预约、未签约导航），见 2b | `services/qqmap.ts` 的 `searchNearby` 重新承担「未签约车场数据源」（首页/搜索页经 `fetchLotsAround`） |
