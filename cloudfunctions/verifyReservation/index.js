...
// 本轮做后两级，OCR 留 Plan 4。只做入场、不做出场（§10：出场识别列入扩展，
// 不做不影响闭环）。
//
// 两种入口：
...
// method 'code' 输码 / 'manual' 手动，operatorId 是操作人）。
...
exports.main = async (event) => {
  ...
  const { lotId, verifyCode, plateNo, method } = event || {}
  if (typeof lotId !== 'string' || lotId === '') {
    return { code: 'BAD_REQUEST', message: '缺少车场' }
  }
  if (method !== 'code' && method !== 'manual') {
  ...
  if (method === 'manual' && (typeof plateNo !== 'string' || plateNo === '')) {
  ...
  try {
    await entryLogs.add({
      data: {
        ...
        at: now,
        method, // 'code' | 'manual'（数据模型 §4，OCR 的 'plate' 留 Plan 4）
        operatorId: OPENID,
        confidence: null,
        imageFileID: null,
      },
